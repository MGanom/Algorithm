...
function solution(n, build_frame) {
  ...
  function pillarSafe(array, x, y) {
    let isSafe = false;
    if (array.find((a, b) => String(x) + String(y - 1)) !== undefined) {
      isSafe = true;
    }
    return isSafe;
  }

  function plateSafe(array, x, y) {
    let isSafe = false;
    if (array.find((a, b) => String(x) + String(y - 1) !== undefined)) {
      isSafe = true;
    }
    if (array.find((a, b) => String(x + 1) + String(y - 1) !== undefined)) {
      isSafe = true;
    }
    return isSafe;
  }
